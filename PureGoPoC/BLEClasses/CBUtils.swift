...
func getActorFromPeripharalUUID(strPeripharalUUID : String) -> BLEActor? {
    for objActor in appDelegate_.deviceActors {
        if let peripharalUUID = objActor.state[BLEConstants.kPeripheralUUIDKey] as? String {
...
func reconnectWithActor(_ deviceActor: BLEActor) {
    ...
    if peripheral == nil {
        ///start ble scanning
        //
        let uuid = UUID(uuidString: deviceActor.state[BLEConstants.kPeripheralUUIDKey] as! String)
//        print(#function,"UUID \(String(describing: uuid?.uuidString))")
    ...
    appDelegate_.centralManagerActor.centralManager?.retrieveConnectedPeripherals(withServices:[]).forEach({ (p) in
        if (p.identifier.uuidString == (deviceActor.state[BLEConstants.kPeripheralUUIDKey]) as! String) {
            appDelegate_.isReconnecting = true
            appDelegate_.centralManagerActor.add(p)
        }
    })
}
...
func CharacteristicPathWithArray(_ arr: [String]) -> String {
    return (arr as NSArray).componentsJoined(by: BLEConstants.kCBPathDelimiter)
}
...
//        print(#function,"finding property \(serviceUUID)")
    ...
    if characteristicUUIDStr != nil {
        return ([serviceUUIDStr!, characteristicUUIDStr!] as NSArray).componentsJoined(by: BLEConstants.kCBPathDelimiter)
    }
    return nil
}
...
extension String {
    ...
    func toHexString() -> String {
       ...
       func hexToString() -> String? {
           ...
           // Ensure there's an even number of characters
           if hex.count % 2 != 0 {
               return nil
           }
           
           // Process pairs of hexadecimal characters
           var index = hex.startIndex
           while index < hex.endIndex {
               let nextIndex = hex.index(index, offsetBy: 2)
               let byte = String(hex[index..<nextIndex])
