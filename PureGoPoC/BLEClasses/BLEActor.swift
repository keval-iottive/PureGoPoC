...
var serialNumberStatic:String = "5844"
let kDevicesStorageKey = "SensorCellDevices"
...
extension CBPeripheral {
    func characteristic(withPath path: String)  -> CBCharacteristic? {
        let pathUUIDs: [CBUUID] = CBUUIDsFromNSStrings(strings: path.components(separatedBy: BLEConstants.kCBPathDelimiter))
...
extension CBCharacteristic {
    func path() -> String {
        return  (NSStringsFromCBUUIDs(cbUUIDs: [service!.uuid, uuid]) as NSArray).componentsJoined(by: BLEConstants.kCBPathDelimiter)
    }
...
//            Loader.hide()
    ...
    func peripheralFound(peripharal : CBPeripheral, metaData : NSMutableDictionary?) {
        ...
        if (deviceActor == nil) {
            ...
            PostNoteBLE("NewDeviceFound", deviceActor as Any)
        }
        else {
            deviceActor.setPeripheral(peripharal)
        }
...
//        }
//            print("macAddressString is :- ", macAddressString)
            ...
            let objBLEModel : BlePeripharalModel = BlePeripharalModel.init(UUID(), peripheral, "", deviceName, nil,0, false, isConnectable, Int(Date().timeIntervalSince1970), [Double(truncating: RSSI)], intRSSIVal , false)
...
class BLEActor: NSObject {
    ...
    var teamId:Int = 0
    var arrBlePeripharalModel: [BlePeripharalModel] = []
    ...
    init(deviceState aState:NSMutableDictionary, servicesMeta aServicesMeta:NSDictionary, operationsMeta aOperationsMeta:NSDictionary, deviceName:String = "") {
        super.init()
        self.state = aState
    ...
    func isActor(_ peripheral: CBPeripheral) -> Bool {
        ...
        return ((state[BLEConstants.kPeripheralUUIDKey] as? String) == peripheralUUID)
    }
    ...
    func setPeripheral(_ peripheral: CBPeripheral) {
        ...
        self.state[BLEConstants.kPeripheralUUIDKey] =  peripheral.identifier.uuidString
        
        if (peripheral.name != nil) {
            state[BLEConstants.kDeviceName] = peripheral.name
        }
        if peripheral.state == .connected {
    ...
    func propertyMeta(for characteristic: CBCharacteristic) -> [AnyHashable: Any]? {
        ...
        let components: [String] = characteristicPath.components(separatedBy:BLEConstants.kCBPathDelimiter)
    ...
    @objc func PeripheralStateChanged(_ note: Notification) {
        ...
        if peripheral?.state == .connected {
            if self.state[BLEConstants.kPeripheralUUIDKey] != nil && ((peripheral?.identifier) != nil) {
                state[BLEConstants.kPeripheralUUIDKey] = peripheral?.identifier.uuidString
            }
    ...
    @objc func PropertyUpdated(_ note: Notification) {
        ...
        if isCommand(inProgressPerformingOperation: BLEConstants.kPropertyOperationRead, onProperty: name) {
            processReadCommandOperation(withValue: val)
        }
        else if isCommand(inProgressPerformingOperation: BLEConstants.kPropertyOperationReadWait, onProperty: name) {
            //            print(#function,"Property Updated -- \(name)")
    ...
    func continueCommandInProgress()
    {
        ...
        commandTimeoutTimer = Timer.scheduledTimer(timeInterval: BLEConstants.kCommandPropertySetTimeout, target: self, selector: #selector(self.commandTimeout(_:)), userInfo: nil, repeats: false)
        ...
        if (NSString.init(string:BLEConstants.kPropertyOperationWrite).isEqual(to:strOperationType as String)) {
            self.writeProperty(nextOperation.value(forKey: "name") as! String, withValue: nextOperation.value(forKey: "value") as Any)
        }else if (NSString.init(string:BLEConstants.kPropertyOperationReadWait).isEqual(to:strOperationType as String)) {
    ...
    func performCommand(_ command: String, withParams params: NSMutableDictionary, metaValue: NSDictionary! = nil) {
        ...
        for obj in arrProperty {
            ...
            if (NSString.init(string:BLEConstants.kPropertyOperationWrite).isEqual(to:strOperationType as String) && !(propertyOperation.object(forKey: "value") != nil)) {
    ...
    @objc func PeripheralDidWriteValueForCharacteristic(_ note: Notification) {
        ...
        if isCommand(inProgressPerformingOperation: BLEConstants.kPropertyOperationWrite, onProperty: property) {
            continueCommandInProgress()
            return
        }
    ...
    func transformValue(_ val: Any, ofProperty name: String, direction dir: kValueTransformDirection) -> Any {
        let commandName =  self.commandInProgress[BLEConstants.kName] as? String
        if (commandName != nil) {
            switch commandName {
//            case BleCommands.kwriteCommandVzbel:
            case BleCommands.kwriteCommandBodyTemp:
                return sendRequestDataValue(val, withDirection: dir)
    ...
    func sendRequestDataValue(_ val: Any, withDirection direction: kValueTransformDirection) -> Any {
        if kValueTransformDirection.In == direction {
            return val
        }
        var payloadData = Data.init()
        let dictionary = val as! Dictionary<String, Any>
        let data = dictionary[BleCommands.kwriteCommandBodyTemp] as! Data
